...
| `Esc` | 暂停游戏并打开调试面板 |
| 按住 `Tab` | 展开当前伤害、移动速度、攻击范围和五类碎片等级 |
| `1`–`6` / `Q` | 直接选择对应武器 / 大力掌 |
...
| 境界 | 移动与空格能力 | 战斗能力 | 天劫 |
| --- | --- | --- | --- |
| 练气 1–9 | 地面跑步；空格翻滚 | 翻滚期间无敌且不能攻击；大力掌攻击最近敌人 1 次 | 3 道雷；预备时间为标准值 3 倍 |
| 筑基 1–9 | 空格短暂跃起并驭空滑行，经历上升、悬停、下降和落地 | 大力掌快速连续攻击最近敌人 2 次并轻微击飞 | 6 道雷；预备时间为练气雷劫的 0.75 倍 |
| 金丹 1–9 | 持续御空飞行；空格切换灵气护盾，进入境界时默认关闭 | 护盾开启时消耗灵气抵挡伤害；大力掌快速连续攻击最近敌人 6 次并明显击飞 | 12 道雷；预备时间为练气雷劫的 0.35 倍 |
| 元婴 1–9 | 持续御空；空格切换灵体出窍 | 灵体状态开启灵气护盾、造成 200% 伤害并承受 150% 伤害；灵气耗尽后伤害穿透但不会跌境；大力掌快速连续攻击最近敌人 8 次并可秒杀普通敌人 | 突破元婴 9 层时仅有 1 道必中的即死雷 |
...
- 大力掌会快速连续出掌，每一掌发出时都重新锁定攻击范围内最近的敌人。
...
- 远程飞行怪：身侧固定悬浮一把带亮紫描边的飞剑；攻击就绪且目标
  进入外层预警距离后，飞剑转向目标并显示低透明度瞄准线。实色线段
  会按目标到攻击范围的距离逐渐填满，目标踏入攻击范围后发射直线弹体。
  基础攻击范围约 180；
  普通怪的实色瞄准线为淡紫色，精英为亮红色；不再显示旧虚线或玩家
  身上的圆形锁定准星。
  普通怪使用无残影飞剑，精英使用更高速且带三层暗红残影的飞剑；冷却
  阶段不显示范围或瞄准线，命中仅播放十字刀光和玩家红白闪烁。
...
- `tests/enemy_ranged_attack_visual_smoke.gd`
...
godot --headless --path . --script res://tests/enemy_ranged_attack_visual_smoke.gd --log-file .godot/enemy-ranged-visual-smoke.log
```
